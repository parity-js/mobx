...
const devLogsFactory = (...params) => {
  let instance = null;

  return class DevLogsStore {
    @observable devLogs = [];
    ...
    @action
    addParsedLog = log => {
      const parsedLog = /^(\d{4}.\d{2}.\d{2}.\d{2}.\d{2}.\d{2})(.*)$/i.exec(
        log
      ); // Get the date inside the log
      if (!parsedLog) return;
